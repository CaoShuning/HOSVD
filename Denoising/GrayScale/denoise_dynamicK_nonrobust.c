...
{
	// useful variables
	int i,j,k,l,k1,k2,num_neighbors=0,minindex_y,minindex_x;
...
label1:				count++;
			...
			{
				num_neighbors = 1;
				nearestNeigh[0][0] = minindex_y;  nearestNeigh[0][1] = minindex_x;
				//printf ("\nWTF"); fflush (stdout);
			}
			...
			//for(k=0;k<num_neighbors;k++)
			{
				...
				nonzero = ps*ps;
				THRESHOLD = SIGMA*sqrt(2*log(ps*ps));
				for(k1=0;k1<ps;k1++)
				{
					for(k2=0;k2<ps;k2++)
					{
...
int main (int argc, char **argv)
{
	...
	for (i = 0; i < numIters;i++)
	{
		...
		fp = fopen ("RESULTS_DYNAMIC_K.TXT","a"); 
		if (!fp) 
		{
			fp = fopen ("RESULTS_DYNAMIC_K.TXT","w");
		}
