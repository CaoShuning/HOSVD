...
void selectbestpatchsize_denoise_routine (double **im, double **refim, double **opim, int H, int W, 
int smallest_scale, int biggest_scale, char *filename, double **imt)
{
	double pr;
	double data1[biggest_scale*biggest_scale],data2[biggest_scale*biggest_scale],ksstat,prob[(2*SR+1)*(2*SR+1)];
	// useful variables
	...
	gsl_vector *gEv,*work;
	FILE *fp;
	double mse[biggest_scale+1],psnr[biggest_scale+1],Kval[biggest_scale+1],Pval[biggest_scale+1],cc[biggest_scale+1];
	...
	for (ps = smallest_scale; ps <= biggest_scale; ps++)
	{
		printf ("\nSCALE = %d",ps); 
		printf ("\n****************************************");	
		fflush(stdout);
...
label1:					count++;
		...
		mse[ps] = MSE(opim,imt,H,W); psnr[ps] = PSNR(mse[ps]); 
		printf ("\nAfter denoising, MSE = %lf, PSNR = %lf",mse[ps],psnr[ps]); 
		fflush (stdout);

		sprintf (opfname,"smoothed_P%dx%d_Sig%.2lf_D%.2lf_iter%d_%s",ps,ps,SIGMA,DISTANCE_THRESHOLD,i,filename);
		writePGM (opfname, opim, H, W);
		compute_residuals (refim,im,opim,H,W);
		sprintf (opfname,"residual_P%dx%d_Sig%.2lf_D%.2lf_iter%d_%s",ps,ps,SIGMA,DISTANCE_THRESHOLD,i,filename);
		...
		KSTest_Noiseness_multiscale (refim,H,W,8,16,&Kval[ps],&Pval[ps]);
		cc[ps] = corrcoeff_Noiseness_multiscale(refim,H,W,8,16);
		printf ("\nK = %lf, P = %lf, cc = %lf",Kval[ps],Pval[ps],cc[ps]);
		fflush(stdout);

		fprintf (fp,"%s %d %lf %lf\n",filename,ps,psnr[ps],cc[ps]);
	...
	for(ps=smallest_scale;ps<=biggest_scale;ps++)
	{
		if (mse[ps] <= minval) { minval = mse[ps]; minindex = ps;}
	}
	printf ("\nMSE: %lf %d ",minval,minindex); fflush(stdout);

	fprintf (fp,"%s %lf %d ",filename,minval,minindex);

	minval = INFINITY;
	for(ps=smallest_scale;ps<=biggest_scale;ps++)
	{
		if (cc[ps] <= minval) { minval = cc[ps]; minindex = ps;}
	}
	printf ("\nCC: %lf %d ",minval,minindex); fflush(stdout);

	fprintf (fp,"%lf %d ",minval,minindex);

	minval = INFINITY;
	for(ps=smallest_scale;ps<=biggest_scale;ps++)
	{
		if (Kval[ps] <= minval) { minval = Kval[ps]; minindex = ps;}
	}
	printf ("\nK: %lf %d ",minval,minindex); fflush(stdout);
	fprintf (fp,"%lf %d ",minval,minindex);

	minval = INFINITY;
	for(ps=smallest_scale;ps<=biggest_scale;ps++)
	{
		if (Pval[ps] <= minval) { minval = Pval[ps]; minindex = ps;}
	}
	printf ("\nlogP: %lf %d ",minval,minindex); fflush(stdout);
	fprintf (fp,"%lf %d ",minval,minindex);
...
int main (int argc, char **argv)
{
	...
	for (i = 0; i < numIters;i++)
	{
		printf ("\n"); fflush(stdout);
		time (&start);
		selectbestpatchsize_denoise_routine (im,refim,im2,H,W,3,16,argv[2],imt);
		time (&end);
