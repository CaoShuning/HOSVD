...
void dctFilter (double **im, double **im2, int ps, int H, int W)
{
	int i,j,k1,k2;
	double a,nz,avgnz=0;
	double **U, **UT,**B, **T1, **S;
	...
	for(i=0;i<H-ps+1;i=i+1)
	{
		...
		for(j=0;j<W-ps+1;j=j+1)
		{
			...
			for(k1=0;k1<ps;k1++)
			{
				for(k2=0;k2<ps;k2++)
				{
					if (fabs(S[k1][k2]) < THRESHOLD) {S[k1][k2] = 0; nz = nz+1.0;}
				}
			}
			nz = nz/(ps*ps); avgnz += nz;

			matrix_multiply(U,S,T1,ps,ps,ps); // T1 = U*S
			matrix_multiply(T1,UT,B,ps,ps,ps); //B = T1*U'				
					
			addPatch(B,im2,numcount,i,j);
/*			mse = 0;
			for(k1=0;k1<ps;k1++)
			{
				for(k2=0;k2<ps;k2++)
				{
					mse += (B[k1][k2]-refim[i+k1][j+k2])*(B[k1][k2]-refim[i+k1][j+k2]);
				}
			}			
			mse /= (ps*ps);
			fprintf (fp,"%.3lf ",mse);*/
	...
	printf ("\nAverage number of zeroes = %lf",avgnz/(H*W)); fflush(stdout);

	for(i=0;i<H;i++)
	{
		for(j=0;j<W;j++)
		{
			a = im2[i][j] /(double)numcount[i][j];

			if (numcount[i][j] == 0)
			{
				printf ("\n%lf %d [%d %d]",im2[i][j],numcount[i][j],i,j); fflush(stdout);
			}
