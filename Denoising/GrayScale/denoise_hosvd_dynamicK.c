				...
				for(l=left;l<=right;l++)
				{
					...
					for (p1=0;p1<ps;p1++)
					{
						for(p2=0;p2<ps;p2++)
						{
							...
							distances[count] += a*a;
							if (distances[count] > DISTANCE_THRESHOLD) {p2 = ps; p1 = ps;}
						}
					}
...
//				addPatch(B,opim,numcount,minK[k][0],minK[k][1],1/((double)cnt1+0.001));
	...
	averageKval /= (H-ps+1)*(W-ps+1);
	printf ("\nAverage value of K is %f",averageKval);
	fflush (stdout);
...
int main (int argc, char **argv)
{
	...
	MAXK = atoi(argv[6]);
	printf ("\nNoise std. dev. = %lf, K = %d, patchsize = %d, file %s, thresh = %lf",SIGMA,MAXK,ps,argv[2],THRESHOLD);
	fflush(stdout);
	...
	for (i = 0; i < numIters;i++)
	{
		...
		sprintf (opfname2,"RESULTS_HOSVD_DYNAMICK_sig%d.TXT",(int)SIGMA);
		fp = fopen (opfname2,"a");
		if (!fp) fp = fopen (opfname2,"w");
